<script>
  ...
  // summary.json carries per-range values: time/timeDay/timeWeek/... and
  // uptime/uptimeDay/uptimeWeek/... ("all" maps to the bare field).
  const fieldFor = (base) => {
    if (!summary) return "";
    const key = range === "all" ? base : `${base}${range[0].toUpperCase()}${range.slice(1)}`;
    return summary[key] !== undefined ? summary[key] : summary[base];
  };
</script>
